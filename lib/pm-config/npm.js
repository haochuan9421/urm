...
const { registryKey, scopeRegistryKeyReg } = require("../const");
...
class npmConfig {
  ...
  async getCurRegistries() {
    return npm.load().then(() => {
      ...
      for (const key in npmCliConfig) {
        ...
        if (scopeRegistryKeyReg.test(key)) {
      ...
      const curRegistries = registryKeys.map((key) => ({
        ...
        // registry 的作用范围
        scope: key === registryKey ? "" : key.substr(0, key.lastIndexOf(":")),
  ...
  async setRegistry(scope, value, where) {
    const key = scope ? `${scope}:registry` : registryKey;
